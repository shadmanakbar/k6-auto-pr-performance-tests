...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === "generate_k6_script") {
            const { prompt, techStack, targetUrl = "http://localhost:8080" } = args as any;
      ...
      Thresholds: p(95)<500, rate<0.01. Use groups and checks.`;
        ...
        if (name === "run_k6_test") {
            const { scriptContent, outputDir = "k6-results" } = args as any;
        ...
        if (name === "format_results") {
            const { resultsJson, exitCode, stack } = args as any;
